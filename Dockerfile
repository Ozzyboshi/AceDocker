...
FROM ozzyboshi/bebbo-amiga-gcc:latest as build-env
...
RUN cd ACE/ && mkdir build
WORKDIR /root/ACE/build
RUN M68K_TOOLCHAIN_PATH=/bin cmake .. -DCMAKE_TOOLCHAIN_FILE=/root/AmigaCMakeCrossToolchains/m68k.cmake -DM68K_TOOLCHAIN_PATH=/opt/amiga -DM68K_CPU=68000 -DM68K_FPU=soft -DCMAKE_BUILD_TYPE=Debug
RUN make
## End of ace debug
